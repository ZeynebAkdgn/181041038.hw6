...
// Insert solution to programming exercise 1, section 5, chapter 9 here
    ...
    private int binarySearch(E item, E[]  data, int i, int size ) {
        ...
        int right = size-1;
        int medium;
        if ( item.compareTo(data[0]) < 0 ) { // before first
            return 0;
        }
        else if (  item.compareTo(data[size-1]) > 0 ) { // after last
            return size;
        }
        else {  // between first and last
            while (true) {
                medium = (left + right) / 2;
                if (item.compareTo(data[medium]) == 0) {
                    return medium;
                }
                if (item.compareTo(data[medium]) < 0) {
                    right = medium;
                } else if (item.compareTo(data[medium]) > 0) {
                    left = medium;
                }
                if ( right -left==1) {
                    return left+1;
